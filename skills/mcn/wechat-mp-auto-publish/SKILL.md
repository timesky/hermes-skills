...
```
17:00定时触发 → 抓取热搜 → ⭐提取摘要 → 推荐主题 → 【人工确认】 → 生成文章 → ⭐去AI化 → 配图 → 发布草稿
    自动           自动        自动        自动        唯一交互       自动      自动     自动     自动
```
...
| 新步骤 | 技能 | 目的 |
|--------|------|------|
| **提取摘要** | summarize-pro | 深度理解热搜话题，不只是标题，提升选题质量 |
| **去AI化** | humanizer-zh | 去除AI写作痕迹，使文章更自然，提升阅读体验 |
